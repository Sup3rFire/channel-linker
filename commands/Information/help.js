  ...
  if (!args.length) {
    ...
    if (command) {
      ...
      if (command.info.description) {
        commandEmbed.setDescription(command.info.description);
      }
      if (command.info.aliases) {
        commandEmbed.addField("Aliases", command.info.aliases.join(", "));
      }
      if (command.info.usage) {
        commandEmbed.addField(
          "Usage",
          client.prefix[0] + command.info.name + " " + command.info.usage
        );
      }
...
module.exports.info = {
  name: "help",
  description:
    "List all categories, commands in a category, or info about a specific command",
  aliases: ["commands"],
